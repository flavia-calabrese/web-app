  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [question.email], (err, row) => {
      if (err) reject(err);
        ...
        db.run(
          sql,
          [question.text, row.id, question.date.toISOString()],
          function (err) {
            if (err) reject(err);
    ...
    db.all(sql, [this.id], (err, rows) => {
      if (err) reject(err);
        ...
        const answers = rows.map(
          (ans) => new Answer(ans.id, ans.text, ans.email, ans.date, ans.score)
        );
        resolve(answers);
      }
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [answer.email], (err, row) => {
      if (err) reject(err);
        ...
        sql =
          "INSERT INTO answer(text, authorId, date, score, questionId) VALUES (?, ?, DATE(?), ?, ?)";
        db.run(
          sql,
          [
            answer.text,
